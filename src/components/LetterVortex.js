...
// Unified ellipse aspect across all rings, based on ring 0 visual
...
const OrbitingLetters = ({ ringRadii = [1.35, 1.42, 1.5], aspect = DEFAULT_ORBIT_ASPECT }) => {
  ...
  useFrame((state, delta) => {
    ...
    // Простые фиксированные радиусы от маленького к большему
    const r0 = 1.35; // самый маленький (внутреннее кольцо)
    const r1 = 1.42; // средний (среднее кольцо)
    const r2 = 1.5;  // самый большой (внешнее кольцо)
    ...
    for (let ring = 0; ring < 3; ring++) {
      ...
      for (let k = 0; k < indices.length; k++) {
        ...
        const radius = ringRadii[ring] ?? ringRadii[ring % ringRadii.length];
        const angle = (index / lettersLen) * Math.PI * 2 + ringAngles[ring];
        const [baseX, baseY] = getEllipsePosition(radius, angle, aspect);
      ...
      {ringIndices.map((indices, ring) => (
        <group key={ring} ref={(el) => (ringRefs.current[ring] = el)}>
          {indices.map((index, positionInRing) => {
            ...
            // Простые фиксированные радиусы от маленького к большему
            const r0_init = 1.35; // самый маленький (внутреннее кольцо)
            const r1_init = 1.42; // средний (среднее кольцо)
            const r2_init = 1.5;  // самый большой (внешнее кольцо)
            const radius = ringRadii[ring] ?? ringRadii[ring % ringRadii.length];
...
const LetterVortex = ({ ringRadii = [1.35, 1.42, 1.5], aspect = DEFAULT_ORBIT_ASPECT }) => {
  return (
